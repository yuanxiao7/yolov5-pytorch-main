...
def log_average_miss_rate(precision, fp_cumsum, num_images):
    """
        ...
        output:
                lamr | log-average miss rate  # 对数平均漏检率  fppi
                mr | miss rate  漏检的占比  描述检测结果中的漏检率
                fppi | false positives per image  每张图片的假正例  占比  每张图片的误检率
    ...
    fppi = fp_cumsum / float(num_images)   # fppi=fp误检数/总图片数
    mr = (1 - precision)    #

    fppi_tmp = np.insert(fppi, 0, -1.0)  # 在fppi_tmp的dim=0插入-1.0
    mr_tmp = np.insert(mr, 0, 1.0)  # 在mr前dim=0插入1.0

    ref = np.logspace(-2.0, 0.0, num = 9)  # 10的-2次方到10的0次方的等比数列，这个等比数列的长度是9个元素。
    for i, ref_i in enumerate(ref):
        j = np.where(fppi_tmp <= ref_i)[-1][-1]  # 取最后的一个值
        ref[i] = mr_tmp[j]

    lamr = math.exp(np.mean(np.log(np.maximum(1e-10, ref))))
    # 1比较返回最大的 2以10为底的对数运算 3对数组元素取均值 4返回e的均值次幂，如 假设均值为3 返回e-3

    return lamr, mr, fppi  # 这三个指标越小越好
...
def draw_text_in_image(img, text, pos, color, line_width):
    ...
    cv2.putText(img, text,  # 参数 图片 要添加的文字 文字添加到图片的位置 字体的类型 字体的大小 字体的颜色 字体的粗细
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        F1 = sum_F1 / n_classes
        text1    = "F1 = {0:.2f}%".format(F1*100)
        print(text1)

        rec = sum_rec / n_classes
        text2 = "recall = {0:.2f}%".format(rec)
        print(text2)

        prec = sum_prec / n_classes
        text3 = "precision = {0:.2f}%".format(prec)
        print(text3)
