...
from utils.utils import cvtColor, preprocess_input, Cutout
from torchvision.transforms import *

class YoloDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, anchors, anchors_mask, epoch_length, \
                 mosaic, mixup, mosaic_prob, mixup_prob, cutout, train, special_aug_ratio=0.7):
    ...
    def __getitem__(self, index):
        ...
        #   这里进行随机擦除
        # ----------------------------------#
        if self.cutout and self.rand() < self.mosaic_prob:
            transform = Compose([
                transforms.ToTensor(),
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, jitter=0.3, hue=.1, sat=0.7, val=0.4):
        ...
        # ])
        # # transform暂且把他看作一个已经实例化的函数，将多步骤给合在一起的函数
        # new_image = transform(img=new_image)
