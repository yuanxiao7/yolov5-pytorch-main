...
if __name__ == "__main__":
    ...
    #                   在除cspdarknet的其它主干中仅影响panet的大小，这个词参数值用于判断下载的主干网络
    #------------------------------------------------------#
    phi             = 's'
    #------------------------------------------------------------------#
    #   mosaic              马赛克数据增强。
    ...
    weight_decay        = 0
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有step（只有下降）、cos（有上有下）
    #------------------------------------------------------------------#
    lr_decay_type       = "cos"
    #------------------------------------------------------------------#
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            wanted_epoch = wanted_step // (num_train // Unfreeze_batch_size) + 1  # 数据集建议调整
    ...
    if True:
        ...
        #---------------------------------------#
        #   根据optimizer_type选择优化器  hasattr 如果该对象又该属性返回Ture
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        for k, v in model.named_modules():
            if hasattr(v, "bias") and isinstance(v.bias, nn.Parameter):  # 卷积神经网络的bias
                pg2.append(v.bias)
            if isinstance(v, nn.BatchNorm2d) or "bn" in k:      # 卷积神经网络归一化的weights
                pg0.append(v.weight)    
            elif hasattr(v, "weight") and isinstance(v.weight, nn.Parameter):  # 卷积神经网络的weights
                pg1.append(v.weight)   
        optimizer = {
