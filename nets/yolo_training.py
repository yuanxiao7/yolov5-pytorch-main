...
class YOLOLoss(nn.Module):
    ...
    def clip_by_tensor(self, t, t_min, t_max):  # 规定数据范围 也可认为数据归一化
        t = t.float()
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w):
        ...
        # 生成先验框的宽高
        scaled_anchors_l = np.array(scaled_anchors)[self.anchors_mask[l]]  # 根据对应层数l去除对应的先验框
...
def weights_init(net, init_type='normal', init_gain = 0.02):
    def init_func(m):
        classname = m.__class__.__name__  # 实例指向自己的类，调用其它类的属性
        if hasattr(m, 'weight') and classname.find('Conv') != -1:
            if init_type == 'normal':
                ...
                raise NotImplementedError('initialization method [%s] is not implemented' % init_type)  # up主多写了not
...
def  get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.05, warmup_lr_ratio = 0.1, no_aug_iter_ratio = 0.05, step_num = 10):
    ...
    if lr_decay_type == "cos":
        warmup_total_iters  = min(max(warmup_iters_ratio * total_iters, 1), 3)  # 第一次取3
        warmup_lr_start     = max(warmup_lr_ratio * lr, 1e-6)   # first 0.1
        no_aug_iter         = min(max(no_aug_iter_ratio * total_iters, 1), 15)  # 5
