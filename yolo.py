...
'''
训练自己的数据集必看注释！  这里定义的这个yolo的类是 将训练好的权值放在这个类里初始化，然后进行预测的
'''
...
class YOLO(object):
    _defaults = {
        ...
        #   在除cspdarknet的其它主干中仅影响panet的大小  panet自上而下、自上而下的FPN
        # ------------------------------------------------------#
        "phi": 's',
        # ---------------------------------------------------------------------#
    ...
    @classmethod  # 它可以将传进来一个当前类作为第一个参数，n外部调用这个类，是传进来cls类的属性，不是该类的属性则无法识别
    ...
    def __init__(self, **kwargs):  # 接收字典
        ...
        # ---------------------------------------------------#
        hsv_tuples = [(x / self.num_classes, 1., 1.) for x in range(self.num_classes)]  # 一个tuple，下面的*x的*是解开列表的
        self.colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))  # hsv转换为rgb
        self.colors = list(map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), self.colors))
        # hsv取值范围在[0,1]，而RBG取值范围在[0,255]，所以乘上255  255红色
        self.generate()

        show_config(**self._defaults)  # 这里调用函数，将参数字典展示出来
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        # print(np.shape(image)[0:2])
        # (1330, 1330, 3)
        # (1330, 1330)  [0:2]由0dim取到1dim
        ...
        # ---------------------------------------------------------#
        #   给图像增加灰条，实现不失真的resize，可以保留更多的图片信息，但同时加入了灰色，也会有一定的影响的
        #   也可以直接resize进行识别，虽然图片变形了，但是输进网络的只有原图片的信息
        # ---------------------------------------------------------#
        image_data = resize_image(image, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        # print("letterbox_image", self.letterbox_image)  Ture bool代数用于判断resize方式
        # ---------------------------------------------------------#
        #   添加上batch_size维度
        # ---------------------------------------------------------#
        image_data = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 0)
        # print("a",image_data.shape)  # [1, 3, 640, 640]
        ...
        for i, c in list(enumerate(top_label)):
            ...
            box = top_boxes[i]  # 按照相应的索引取出第i个框和它对应的得分
            score = top_conf[i]
            # print("i: , c: ", i, c)
            ...
            i:, c: 0 0
            person
            [550.01013  69.82842 940.0668  279.40723]
            0.9016482
            ...
            if top - label_size[1] >= 0:
                ...
                # print("tx1: ", text_origin)  # tx1:  [ 69 510]
            else:
                text_origin = np.array([left, top + 1])
                # print("tx2: ", text_origin)

            for i in range(thickness):
                draw.rectangle([left + i, top + i, right - i, bottom - i], outline=self.colors[c])
                # print(self.colors[c])  # (255, 0, 0)  画预测框
            draw.rectangle([tuple(text_origin), tuple(text_origin + label_size)], fill=self.colors[c])
            # print(tuple(text_origin))  # (69, 510)左下角  画顶部显示的矩形框，并填充颜色
            # print(tuple(text_origin + label_size))  # (289, 550)右上角
            draw.text(text_origin, str(label, 'UTF-8'), fill=(0, 0, 0), font=font)
            # print(font)  # 放字体到矩形框text_origin里
            del draw
